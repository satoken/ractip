...
RUN git clone https://github.com/ERGO-Code/HiGHS \
    && cd HiGHS \
    && cmake -DFAST_BUILD=ON -DCMAKE_BUILD_TYPE=Release -DSHARED=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=OFF -G Ninja -B build \
    && cmake --build build \
...
FROM alpine:latest
COPY --from=builder /usr/local/bin/ractip /usr/local/bin/ractip